...
async function fetchUserContributions(username) {
  ...
  if (!response.ok) {
    throw new Error(`Failed to fetch contributions for ${username}: ${response.status}`);
  }
...
function mergeContributions(usersData) {
  ...
  const counts = Array.from(dateMap.values()).map(d => d.count).filter(c => c > 0);
  ...
  const totalContributions = usersData.reduce((sum, u) => sum + u.totalContributions, 0);
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!usernames || !Array.isArray(usernames) || usernames.length === 0 || usernames.length > 4) {
